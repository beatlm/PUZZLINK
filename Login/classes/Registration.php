    ...
    private function registerNewUser()
    {
        if (empty($_POST['user_name'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $query_check_user_name = $this->db_connection->query("SELECT * FROM usuarios WHERE user_name = '" . $this->user_name . "';");

                if ($query_check_user_name->num_rows == 1) {
                    $this->errors[] = "Sorry, that user name is already taken. Please choose another one.";
                } else {
                    // write new users data into database
                    $query_new_user_insert = $this->db_connection->query("INSERT INTO usuarios (user_name, user_password_hash, user_email) VALUES('" . $this->user_name . "', '" . $this->user_password_hash . "', '" . $this->user_email . "');");
